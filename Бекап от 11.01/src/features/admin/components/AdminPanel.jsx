...
import { Settings, Users, Server, DollarSign, Edit2, Save, X, Bug, LogOut, Copy, Trash2, CheckCircle2, XCircle, AlertCircle, PlusCircle, TestTube, Loader2, Network, Activity, Link2, Monitor } from 'lucide-react'
        ...
        {adminTab === 'settings' && (
          <div className="space-y-[clamp(1rem,0.8rem+1vw,2rem)]">
            ...
            <div className="bg-slate-900 rounded-lg sm:rounded-xl shadow-xl border border-slate-800 section-spacing-sm">
              ...
              {/* Кнопка сохранения всех настроек - Mobile First */}
              <div className="flex justify-end pt-3 sm:pt-4 border-t border-slate-800 mt-4 sm:mt-5 md:mt-6">
                <button
                  onClick={onHandleSaveSettings}
                  className="btn-icon-only-mobile min-h-[44px] w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-lg sm:rounded-xl transition-all flex items-center justify-center gap-2 text-sm sm:text-base touch-manipulation"
                  aria-label="Сохранить все настройки"
                >
                  <Save className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
                  <span className="btn-text whitespace-nowrap">Сохранить все настройки</span>
                </button>
              </div>
            </div>
          </div>

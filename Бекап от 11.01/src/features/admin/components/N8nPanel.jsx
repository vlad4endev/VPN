...
const N8nPanel = ({ onSaveSettings }) => {
  ...
  // Загрузка webhook URL из настроек (используем settings из контекста или загружаем из Firestore)
  useEffect(() => {
    const loadWebhookUrl = async () => {
      // Если мы только что сохранили URL, не перезагружаем его из settings
      ...
      // Сначала пытаемся использовать settings из контекста
      if (settings) {
        const url = settings.n8nWebhookUrl || settings.webhookUrl || ''
        logger.info('Admin', 'Загрузка webhook URL из контекста settings', { 
      ...
      // Если settings нет в контексте, загружаем из Firestore
      if (!db) {
      ...
      try {
        logger.debug('Admin', 'Загрузка webhook URL из Firestore (settings нет в контексте)')
        ...
        if (settingsSnapshot.exists()) {
          ...
          logger.debug('Admin', 'Webhook URL загружен из Firestore', { 

...
export const dashboardService = {
   ...
   * @returns {Promise<Object>} Обновленные данные пользователя с VPN ссылкой
   */
  async createSubscription(user, tariff, devices = null, natrockPort = null, periodMonths = 1, testPeriod = false, paymentMode = 'pay_now', discount = 0) {
    ...
    // ВАЖНО: Этот вызов должен выполняться ВСЕГДА, независимо от режима оплаты (тестовый период или оплачено)
    ...
    // Создаем запись о платеже (только если оплачено сейчас)
    if (paymentMode === 'pay_now' && !testPeriod) {
      ...
      if (paymentAmount > 0) {
        ...
        logger.info('Dashboard', 'Платеж создан', { 
          userId: user.id, 
          amount: paymentAmount, 
          devices: finalDevices,
          periodMonths: finalPeriodMonths,

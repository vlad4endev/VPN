...
class ThreeXUI {
  ...
  constructor() {
    ...
    this.username = import.meta.env.VITE_XUI_USERNAME
    this.password = import.meta.env.VITE_XUI_PASSWORD
   ...
   * Отправляет POST запрос на /login с username и password согласно документации 3x-ui
   * Сохраняет cookie сессии автоматически благодаря withCredentials
   * @returns {Promise<Object>} Ответ от сервера
   */
  async login() {
    try {
      logger.debug('ThreeXUI', 'Начало авторизации в 3x-ui', { 
        username: this.username ? 'установлен' : 'не установлен',
      ...
      if (!this.username || !this.password) {
        const error = new Error(
          `XUI credentials не настроены в переменных окружения.\n` +
          `Установите в .env файле:\n` +
          `- VITE_XUI_USERNAME=${this.username ? 'установлен' : 'НЕ УСТАНОВЛЕН'}\n` +
          `- VITE_XUI_PASSWORD=${this.password ? 'установлен' : 'НЕ УСТАНОВЛЕН'}\n` +
          `После изменения .env перезапустите dev-сервер (npm run dev)`
        )
        logger.error('ThreeXUI', 'Отсутствуют credentials для авторизации', {
      ...
      // Согласно документации 3x-ui, авторизация выполняется через POST /login
      ...
      // В 3x-ui успешный ответ обычно содержит success: true или код статуса
      const data = response.data || {}
      if (data.success === false || data.success === 0) {
        const error = new Error(data.msg || 'Ошибка авторизации')
        logger.error('ThreeXUI', 'Ошибка авторизации в 3x-ui', { response: data }, error)
        throw error
      }

      logger.info('ThreeXUI', 'Успешная авторизация в 3x-ui')
      return data
    } catch (error) {
      logger.error('ThreeXUI', 'Ошибка авторизации в 3x-ui', {
        status: error.response?.status,
        statusText: error.response?.statusText,
        url: error.config?.url,
        baseURL: this.baseURL
      ...
      if (error.response?.status === 404) {
        errorMessage = `Панель 3x-ui недоступна (404). ` +
          `Проверьте настройки:\n` +
          `- XUI_HOST в .env должен указывать на панель (например: http://localhost:2053)\n` +
        ...
        errorMessage = `Ошибка авторизации (${error.response?.status}). ` +
          `Проверьте правильность VITE_XUI_USERNAME и VITE_XUI_PASSWORD в .env файле.`
      }
...
// Автоматическая инициализация при импорте (опционально, можно вызывать вручную)
// instance.initialize().catch(err => {
//   console.error('Failed to initialize ThreeXUI:', err)
// })

...
# 
# Этот скрипт:
# 1. Получает последние изменения из GitHub (ветка main)
# 2. Перезапускает frontend и backend
#
# Использование:
...
log "🔄 Быстрое обновление кода на сервере"
echo ""
...
# Используем start-all.sh если он существует
if [ -f "start-all.sh" ]; then
    ./start-all.sh
    log_success "Службы перезапущены"
else
    log_warning "Скрипт start-all.sh не найден"
    log_info "Запустите службы вручную: npm run dev"
fi

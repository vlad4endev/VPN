...
export async function chat(messages, options = {}) {
  ...
  if (!apiKey) {
    return { ok: false, error: 'OpenAI API key not set', code: 'NO_API_KEY' }
  }
  ...
  try {
    ...
    const message = err.response?.data?.error?.message || err.response?.data?.message || err.message || 'OpenAI request failed'

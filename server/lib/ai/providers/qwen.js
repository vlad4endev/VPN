/**
 * Alibaba DashScope (Qwen) — OpenAI-совместимый endpoint.
 * Модели: Qwen 3.5 Max (qwen3-max), Qwen Max (qwen-max-latest), Qwen 3.5 Plus и др.
 * По умолчанию: qwen3-max (Qwen 3.5 Max).
 * Ключ: https://dashscope.console.aliyun.com/ или DASHSCOPE_API_KEY в .env
 * Документация: https://help.aliyun.com/zh/model_studio/
 */
...
export async function chat(messages, options = {}) {
  ...
  if (!apiKey) {
    return { ok: false, error: 'DashScope (Qwen) API key not set', code: 'NO_API_KEY' }
  }
  ...
  try {
    ...
    const message = err.response?.data?.error?.message || err.response?.data?.message || err.message || 'Qwen (DashScope) request failed'

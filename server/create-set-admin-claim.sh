#!/bin/bash
# Запустить на сервере в каталоге server/ — создаст set-admin-claim.cjs
# Использование: bash create-set-admin-claim.sh   или: curl -sL URL | bash
set -e
cd "$(dirname "$0")"
cat > set-admin-claim.cjs << 'ENDOFFILE'
#!/usr/bin/env node
/**
...
ENDOFFILE
echo "✅ Создан файл: $(pwd)/set-admin-claim.cjs"
echo "Запуск миграции: node set-admin-claim.cjs --migrate"

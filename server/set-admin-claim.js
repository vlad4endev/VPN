...
try {
  ...
  console.error('Убедитесь, что файл firebase-service-account.json в папке server/ или задайте FIREBASE_SERVICE_ACCOUNT_PATH')
  process.exit(1)
}
...
Использование (из папки server/):
  node set-admin-claim.js <email>         - Установить admin claim для пользователя
  node set-admin-claim.js --migrate       - Мигрировать всех существующих администраторов
  node set-admin-claim.js --info <email> - Показать информацию о пользователе

Примеры:
  node set-admin-claim.js admin@example.com
  node set-admin-claim.js --migrate
  node set-admin-claim.js --info admin@example.com
`)
  process.exit(0)
}
